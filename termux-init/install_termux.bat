...
:: 检查init_termux.sh文件是否存在
echo 检查 init_termux.sh 初始化脚本...
if not exist "init_termux.sh" (
    echo [错误] 未找到 init_termux.sh 文件
    echo 请确保 init_termux.sh 文件与此批处理脚本在同一目录
    echo.
    pause
    goto :manual_install_end
)
...
adb shell mkdir /sdcard/0.file
adb shell mkdir /sdcard/0.file/shell
adb push init_termux.sh /sdcard/ >nul 2>&1
if not errorlevel 1 (
    echo [成功] init_termux.sh 已推送到设备: /sdcard/0.file/shell/init_termux.sh
) else (
    echo [警告] 无法推送 init_termux.sh 到设备
...
echo.
echo 注意事项:
echo - 确保 init_termux.sh 文件与此脚本在同一目录
echo - 确保 Termux 完成初始化后再执行 sh /sdcard/0.file/shell/init_termux.sh
...
echo.
echo 请确保:
echo 1. init_termux.sh 文件在同一目录
echo 2. Termux 应用已正常启动并完成初始化
echo 3. 执行 adb shell 后运行 sh /sdcard/0.file/shell/init_termux.sh
echo.
pause